import{GluonElement,html}from'../gluonjs/gluon.js';import'../overwebs-fonts/overwebs-fonts.js';class OverwebsButton extends GluonElement{get template(){return html`
    <style>
    :host {
    ...
    <slot id="slot"></slot>
    `}static get observedAttributes(){return['disabled']}constructor(){super(),this._links=[],this._buttons=[]}connectedCallback(){super.connectedCallback(),this.$.slot.assignedNodes().forEach((a)=>{a instanceof HTMLAnchorElement?this._links.push(a):a instanceof HTMLButtonElement&&this._buttons.push(a)}),null!==this.getAttribute('disabled')&&this._propagateDisabled(!0)}attributeChangedCallback(a,b,c){'disabled'===a&&this._propagateDisabled(null!==c)}_propagateDisabled(a){a?(this._links.forEach((a)=>{a.tabIndex=-1,a.style.pointerEvents='none'}),this._buttons.forEach((a)=>{a.setAttribute('disabled','')})):(this._links.forEach((a)=>{a.tabIndex=void 0,a.style.pointerEvents=''}),this._buttons.forEach((a)=>{a.removeAttribute('disabled')}))}}customElements.define(OverwebsButton.is,OverwebsButton);
//# sourceMappingURL=overwebs-button.js.map
